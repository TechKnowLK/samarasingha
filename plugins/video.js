...
const DWLOAD_VID = "*🔎Searching Your Song*"
...
var i = Math.floor(60*Math.random())  
    ...
    Kingamda.addCommand({ pattern: 'video ?(.*)', fromMe: tk, deleteCommand: false, desc: Lang.MP4,  deleteCommand: false}, async (message, match) => {
        
    const link = match[1]
        if (!link) return await message.client.sendMessage(message.jid,YT_NEED,MessageType.text)
        await message.client.sendMessage(message.jid,DWLOAD_VID,MessageType.text);
        await axios
          .get(`https://api.lolhuman.xyz/api/ytplay?apikey=${r_text[i]}&query=${link}`)
          .then(async (response) => {
    ...
    Kingamda.addCommand({ pattern: 'video ?(.*)', fromMe: tk, dontAddCommandList: true, deleteCommand: false, desc: Lang.MP4,  deleteCommand: false}, async (message, match) => {
      ...
      const link = match[1]
          ...
          await axios
            ...
            .then(async (response) => {
              ...
              const profileBuffer = await axios.get(info.thumbnail, {responseType: 'arraybuffer'})
              const msg = `*《《───── « ⋅ʚ♡ɞ⋅ » ─────》》*\n   *》》🎧සින්දුවෙ විස්තරෙ🎧《《*\n*《《───── « ⋅ʚ♡ɞ⋅ » ─────》》*`+`\n\n\n`+
                          `*NAME* ➜ *${info.title}*` + `\n\n`+
                          `*VIEWS* ➜ *${info.views}*` + `\n\n`+
                          `*OWNER* ➜ *${info.uploader}*` + `\n\n`+
     ...
     Kingamda.addCommand({ pattern: 'song ?(.*)', fromMe: tk, deleteCommand: false, desc: Lang.MP4,  deleteCommand: false}, async (message, match) => {
