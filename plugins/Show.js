...
Kingamda.addCommand({ pattern: 'twt ?(.*)', fromMe: td, desc: Lang.TWT}, async (message, match) => {

    const userName = match[1]

    if (!userName) return await message.sendMessage(errorMessage("ලින්ක් එකක් දෙන්නො..."))
...
Kingamda.addCommand({ pattern: 'show ?(.*)', fromMe: td , desc: Lang.SHOW}, async (message, match) => {
...
Kingamda.addCommand({ pattern: 'twt ?(.*)', fromMe: true, dontAdCommandList: true}, async (message, match) => {
...
Kingamda.addCommand({ pattern: 'show ?(.*)', fromMe: true , dontAdCommandList: true}, async (message, match) => {
